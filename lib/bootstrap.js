...
exports.initExpressApp = function(config, api, cb) {
	...
	// Define HTTP routes
	app.namespace('/api', apiErrorHandler, api.auth.authenticate, function() {
...
exports.checkPreviousPassphrase = function(config, api, cb) {
	...
	if(oldHmac) {
		...
		if(newHmac !== oldHmac) {
			console.warn("It seems that your passphrase has changed !")
			program.confirm('Continue (y/n) ? ', function(ok) {
				if(ok) return cb();
				process.exit(1);
			});
		} else return process.nextTick(cb);
	} else {
