...
var plugin = function resourcesAccessPlugin(schema, options) {
  ...
  function extendQueryWithPermissions(query, operation, permissions) {
  ...
  schema.methods.limitQueryWithPermissions = function(query, operation, cb) {
    extendQueryWithPermissions(query, operation, this.permissions);
    this.findRolesWithOperation(operation, function(err, roles) {
      if(err) {
        return cb(err);
      }
      roles.forEach(function(r) {
        extendQueryWithPermissions(query, operation, r.permissions);
      });
