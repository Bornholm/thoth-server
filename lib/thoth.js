...
p._askPassphrase = function(cb) {
  ...
  if(program.passphrase) {
    app._passphrase = program.passphrase;
...
p._initModels = function(cb) {
  ...
  var accessRights = this.config.get('accessRights') || {};
  ...
  models.User.setDefaultPermissions(accessRights.defaultPermissions);
  models.User.setDefaultRoles(accessRights.defaultRoles);
...
p._checkAuthBackend = function(cb) {
  ...
  this.logger.info('Auth backend', {backend: authType});
